...
const API_URL = "http://localhost:5000/facturas"; // Ajusta la URL a tu servidor
...
export async function agregarFactura(datosFactura) {
    try {
        ...
        if (!response.ok) {
            ...
            throw new Error("Error al agregar el factura");
        }
...
export async function eliminarFactura(id) {
    try {
        ...
        console.error("error");
        Swal.fire("Error", "No se pudo conectar al servidor para eliminar el factura", "error");
        throw error;
    }
}
